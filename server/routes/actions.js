...
router.get('/sites/:id/actions', async (req, res) => {
  try {
    const site = await db.prepare('SELECT id FROM sites WHERE id = ?').get(req.params.id);
...
router.post('/sites/:id/actions', async (req, res) => {
  try {
    ...
    await db.prepare('UPDATE sites SET date_maj = ? WHERE id = ?').run(now, req.params.id);

    const row = await db.prepare('SELECT * FROM actions WHERE id = ?').get(info.lastInsertRowid);
